...
- `constitution.md`, `spec/contract/openapi.yaml` and `plan/tasks.md`, with `{{PROJECT_NAME}}` and
  `{{DATE}}` filled in;
- `spec/spec.md`, carrying only a note that it is generated by `sdlc run ratify` from
  `spec/domains/*.md` — it has no placeholders to fill, since it is not written by hand;
- `spec/domains/`, seeded empty (holding only `.gitkeep`) for domain files to be added later;
