...
`reverts: <name>`, and the body lists it as restored. An item closed on a row that does not show
its test ran as it stood — a result of another test, or a failure whose test never reached the
target — is reopened by the same pass and listed as reopened. It is not a ruling and asks for no seat. A proposal with
