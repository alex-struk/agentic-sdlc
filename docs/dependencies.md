...
live in `package.json` (the pipeline's own dependencies, which are exactly `yaml` and `ajv`), in a
project's `.sdlc/lock.json` (skill packs, resolved to a commit by `sdlc init`), or in the
project-side files `sdlc init` installs — `templates/project/tests/package.json` for the acceptance
harness, and the compose override the `contract` stage writes for the oracle's images.
...
| `@playwright/test` | The acceptance suite a project owns: `sdlc init` installs `tests/package.json` with this pin, and `src/testrun/playwright.mjs` runs the suite through it | npm dev dependency of the project repository the pipeline produces, exact-pinned in the harness template | `1.63.0` | A calibration result is evidence about the old application, and evidence is only comparable across runs if the thing producing it does not move underneath them: a browser-automation release can change how a locator resolves or how a timeout is reported, which would show up as a criterion changing its result with no change to the criterion, the test or the application. Bumping the pin is a deliberate edit to the template, visible in a diff, and one whose next calibration run re-measures everything. |
| `@playwright/mcp` | The browser `bind-adapter`'s session reaches the running target through, declared by the stage's `mcp` hook and launched with `npx -y @playwright/mcp@<version> --headless --isolated` | MCP server, run on demand by the stage session; never installed into either repository | `0.0.80` | The server's tool names and arguments are part of the stage's prompt contract — the skill text tells the agent which tools exist — so an upstream rename would change what the stage does with no commit in either repository. Pinning is also what keeps `bind-adapter` the only path to the running target: the tool list allows the server's tools and no `Bash`, and an unpinned server is an unreviewed change to that list (`docs/decisions/0006-contract-stage-and-oracle.md`). |
| `typescript` | Type-checking the acceptance suite against the generated surface (`tests/generated/*`), through the harness's `npm run typecheck` | npm dev dependency of the project repository the pipeline produces, exact-pinned in the harness template | `5.9.3` | The generated types are the contract between a blind test and an adapter neither the test's author nor the adapter's author saw: a test that names an action the surface does not declare must fail to compile rather than fail at run time against the oracle, where it would be indistinguishable from a real behavioural difference. A floating minor could change that verdict between two runs. |
| `axllent/mailpit` | The mail catcher the oracle's compose override publishes, so a criterion about an email can be observed without sending one | Docker image named by the compose override the `contract` stage writes (`src/stages/skills/contract.md`), pinned by tag | `v1.28.0` | Its API is what `tests/fixtures/mail.ts` calls (`GET /api/v1/search?query=to:<addr>`, `DELETE /api/v1/messages`), so the image version decides whether a mail-related criterion can be observed at all. A moving tag would let a criterion's result change because the catcher's API moved, which is the one thing calibration must never confuse with the old application's behaviour. |
| Vitest, Storybook | Unit tests and a component catalogue in a generated project | npm dev dependencies of the project repository the pipeline produces | not applicable — added when a project's application code is scaffolded | Standard tooling for this shape of application. |
| Docker Compose v2 | Bringing the old application up as the oracle: `sdlc oracle up|down|status` shells out to `docker compose`, and `sdlc doctor` reports the version it finds | External tool required on the machine, not installed by the pipeline | not applicable — whatever the machine has, reported by `doctor` | Every old application this pipeline is pointed at already ships a compose file, so the oracle needs no new packaging: the pipeline adds an override and chooses ports. The v2 spelling (`docker compose`, not `docker-compose`) is required because the runner passes two `-f` files and a `-p` project name in one invocation. |
