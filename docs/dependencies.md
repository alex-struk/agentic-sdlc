...
| `mattpocock/skills` | `grilling` (the intent interview), `domain-modeling`, `prototype`, `tdd`, `code-review` | Skill pack: cloned into `.sdlc/packs/`, skills copied into `.claude/skills/`, pinned to a commit by `sdlc init` | commit: pinned in Task 15 | Small, composable, model-agnostic skills; `grilling` is the intent interview, `tdd` defines seams, `prototype` produces UI variants. |
| `bcgov/crow` | The `crow-bcgov-ux` skill; structure of the `design/DESIGN.md` template | Skill pack pinned to a commit; template structure copied with attribution | commit: pinned in Task 15 | BC Design System and accessibility guidance written for BC Gov services. |
...
| `bcgov/agent-skills` | `github-actions` hardening, `openshift-deployment` skills | Skill pack pinned to a commit by `sdlc init` | commit: pinned in Task 15 | Org-maintained; the Actions hardening rules are specific and correct. |
| `bcgov/quickstart-openshift` and `-helpers` | Scaffold and deploy workflows for the `openshift-ts` stack profile | Referenced by version once the stack profile exists | not applicable — the stack profile is a later task | Org-maintained, current, and deploys to the platform the profile targets. |
| `DietrichGebert/ponytail` | A scope brake during the build stage | Optional skill pack, disabled by default | commit: pinned in Task 15 | Honest benchmarks report it may hurt with reasoning models, so it is measured before being kept enabled. |
