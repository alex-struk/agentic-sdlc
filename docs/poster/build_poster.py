...
p.cards([
    ...
    {"title": "The marketplace rebuild", "acc": "navy",
     ...
     "lead": "Same behaviour, modern stack, existing Postgres schema kept, sandbox only.",
     "items": ["Old repo is a read-only input named in config.",
               "Rebuilt more than once as the pipeline improves.",
...
p.cards([
    ...
    {"title": "Spec and criteria", "acc": "navy",
     ...
     "items": ["R-12.3 (v2) [confidence] + citations + reconciliation.",
    ...
    {"title": "Contract", "acc": "gold",
     "lead": "The part of the spec tests need to act and observe.",
     "items": ["personas.yaml: roles and how a test signs in.",
               "surface.yaml: routes, titles, actions, observations, test IDs.",
               "openapi.yaml, observables.yaml, seed SQL."]},
], item_size=12.4)
...
p.cards([
    ...
    {"title": "Design spec and catalogue", "acc": "gold",
     ...
     "items": ["Built with the BC Design System React components.",
               ...
               "Component layer is kept: hardened, not rebuilt."]},
...
p.chain([
    ...
    ("build", "One slice, TDD at seams, evidence receipt."),
], highlight=[2, 3])
...
p.chain([
    ...
    ("deploy", "Stack profile workflow to a sandbox namespace. No agent holds a credential."),
...
p.steps_pair(
    left={"label": "Derive and calibrate", "acc": "navy",
          ...
          "steps": [
              ...
              ("Run against the old app", "Compose file, seed loaded, dev-only session route to sign in as a role."),
...
p.cards([
    {"title": "Stack profile openshift-ts", "acc": "navy",
     ...
     "items": ["React, Vite, TanStack Router, BC Design System components.",
               ...
               "Keycloak OIDC plus a sandbox-only test identity.",
...
p.steps_pair(
    left={"label": "Phases 0 to 2: the durable assets", "acc": "gold",
          ...
          "steps": [
              ("0 Harness", "Both repos local, sdlc init, constitution, config, guardrails, persona briefs. Exit: checks green on an empty proposal."),
              ("1 Spec", "intent, archaeology per domain, ratify. Exit: no accepted criterion is inferred or open."),
              ("2 Tests", "derive-tests, bind-adapter old, calibrate. Exit: every calibrate row is pass or ruled."),
          ]},
    right={"label": "Phases 3 to 5: build, then rails", "acc": "navy",
           "sub": "Each slice is a full pipeline cycle and a metrics data point.",
           "steps": [
               ("3 Design", "DESIGN.md and catalogue. Exit: Inderdeep approves."),
               ("4 Build", "plan, then per slice build, verify, review-and-ship, deploy. Exit: every slice done; parity reported."),
               ("5 Rails", "operate metrics, one feature through the full chain, one trivial change through the short circuit, harness fixes. Exit: version bumps; rebuild two starts from the same config."),
           ]})

p.cards([
    {"title": "Open questions", "acc": "gold",
     "items": ["Test identity on the new target, decided at G2.",
               "Observing email on the old app; API surface may be the limit.",
               "Kaegan's pack source and MCP servers are not public; ideas ported, ask anyway.",
               "Ponytail with reasoning models.",
               "Egress is a pattern filter; private run record is the safe default."]},
