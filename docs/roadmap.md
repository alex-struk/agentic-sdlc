...
| 1c · Tests and oracle | The contract completed from the old application and generated into types the tests import, blind test derivation from the spec bundle, adapters per target bound through a browser, the old application started on pipeline-chosen ports as the oracle, calibration with per-criterion rulings | `contract`, `derive-tests`, `oracle`, `bind-adapter`, `calibrate` | in progress |
