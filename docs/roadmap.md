...
| 1b · Spec side | Read-only sources, the criterion format and parser, intent interview, archaeology per domain, ratification with the product-owner's condition vocabulary, coverage board and criteria pages | `intent`, `archaeology`, `ratify` | done |
| 1c · Tests and oracle | The contract completed from the old application and generated into types the tests import, blind test derivation from the spec bundle, adapters per target bound through a browser, the old application started on pipeline-chosen ports as the oracle, calibration with per-criterion rulings | `contract`, `oracle`, `bind-adapter`, `derive-tests`, `calibrate` | in progress |
| 1d · Design and plan | Design spec and screen catalogue against the design system, reviewed by the UX persona; plan with a constitution check, tasks cut into vertical slices, decision records for stack choices | `design`, `plan` | in progress |
| 1e · Build, verify, review | One slice of the plan built by an agent that cannot see the suite, the rebuilt application run locally from its own compose file, that slice's acceptance criteria verified against it, failures routed back to `build --revise` under a retry ceiling that escalates, and review as the reviewer persona's G3 ruling over the built proposal | `sandbox`, `build`, `verify`, `rule --by agent:reviewer` | in progress |
