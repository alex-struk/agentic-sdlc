...
Both start as local git repositories during phase 0 and are pushed to the
bcgov organisation when phase 0 ends, because gates are pull requests from
phase 1 onward and the checkpoint workflow must fetch the pipeline by version.
`bcgov-c` is the private fallback if the project ever needs it.
...
|---|---|---|---|
| G0 intent | Is this the right problem and outcome? | `intent/<slug>.md` | Alex |
...
| G2 plan | Is the architecture sound and every criterion assigned? | `plan.md` | Alex |
| G3 review and ship | Does the PR do what the slice said, with evidence? | PR with review comment and receipt | Alex |
| G-POL policy | May the policy change? | `.sdlc/config.yaml` policy section | Alex |

Every gate is a pull request. The PR body is generated by the stage and leads
with the question, the recommendation, and a link to the page on the state
site. Approval is the PR approval. Return is "request changes" with a comment,
which the runner feeds back to the producing stage as input.

### 6.2 Tiers and the short circuit
...
### 6.3 What "unverified provenance" costs
...
targets:
  new:
    ...
    identity: test-idp
policy:
  gates: {G0: alex, G1: alex, G-DESIGN: inderdeep, G2: alex, G3: alex, G-POL: alex}
...
|---|---|---|---|
| 0 Harness | Create both repos locally, `sdlc init`, constitution, config, guardrails, push to bcgov | Checkpoint workflow green on an empty PR in the project repo | Alex |
