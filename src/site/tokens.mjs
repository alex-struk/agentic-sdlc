// The B.C. Design System token set, verbatim.
//
// Source:  @bcgov/design-tokens 5.0.0, css/variables.css
// Fetched: 2026-09-07
//
// The pipeline takes exactly two dependencies (decision 0001), so the published package
// cannot be consumed at build time and the generated site must be a static file with no
// build step of its own. The B.C. Design System UX guidance covers precisely that case:
// where the official packages cannot be consumed, derive ONE centralised adapter from
// the current published token output and record its source and version. This module is
// that adapter and it is the only place in the pipeline where a resolved colour, size or
// shadow value appears. Every other module refers to these by their semantic name, so a
// token release is a regeneration of this file and nothing else.
//
// To refresh: npm pack @bcgov/design-tokens, take css/variables.css, replace the block
// below with its declarations, and update the version and date above.

export const TOKENS_SOURCE = "@bcgov/design-tokens 5.0.0";
