...
import { parseDomainFile, parseAll, applyConditions, applyCalibrateRulings, mintIds, serialiseDomainFile, writeIndex, renderSpecIndex, compareIds, CONDITION_GRAMMAR, CALIBRATE_GRAMMAR } from "../spec/criteria.mjs";
...
function checkCalibrateTargetOption(ctx) {
  const id = "calibrate-target-option";
  const t = calibrateTarget(ctx);
  if (!t) return { id, ok: false, messages: ["calibrate needs --target <t>, or config.oracle.target for it to default to"] };
  if (t === "old") {
    if (ctx.config?.oracle?.target !== "old")
      return { id, ok: false, messages: ['calibrate: target "old" needs config.oracle (with oracle.target: old)'] };
    return { id, ok: true, messages: [] };
  }
  const targets = ctx.config?.targets ?? {};
  if (!(t in targets))
    return { id, ok: false, messages: [`target "${t}" is not "old" and not in config.targets: ${Object.keys(targets).join(", ") || "(none configured)"}`] };
  if (!targets[t].base_url)
    return { id, ok: false, messages: [`calibrate: target "${t}" has no base_url configured`] };
  return { id, ok: true, messages: [] };
}
...
// up` has no local file to read the URLs out of; the results file still records where
// the run pointed, and the mock test runner never calls either URL.
...
// (`readRedoIds` above). A `test-wrong` ruling adds to it; an id already there is left
// exactly as it is, since the first reason recorded is the one a person wrote about, and
// nothing is ever removed here — `derive-tests` owns clearing an entry it has acted on.
...
function calibrateIndex(projectDir) {
  ...
  if (!existsSync(p)) return { generatedFrom: "", byId: new Map() };
  let index;
  try { index = JSON.parse(readText(p)); } catch { return { generatedFrom: "", byId: new Map() }; }
  return {
...
// A calibration proposal already open — its branch exists with no gate file on it yet —
// is the question this stage is waiting on, so no second one is opened alongside it.
// Returns the highest number seen on a branch or a gate file either way, so the next
// proposal continues the sequence rather than reusing a number a ruled one already holds.
function calibrateFollowUpState(projectDir, target) {
  const pattern = `refs/heads/proposal/calibrate-${target}-*`;
  const refs = gitOk(["for-each-ref", "--format=%(refname:short)", pattern], projectDir)
    ...
    : [];
  const re = new RegExp(`^proposal/calibrate-${escapeRe(target)}-(\\d+)$`);
  let highest = 0;
  let open = null;
  for (const branch of refs) {
    const m = re.exec(branch);
  ...
  const dir = join(projectDir, ".sdlc", "gates");
  if (existsSync(dir)) {
    const gre = new RegExp(`^calibrate-${escapeRe(target)}-(\\d+)\\.yaml$`);
    for (const f of readdirSync(dir)) {
      const m = gre.exec(f);
      if (m) highest = Math.max(highest, Number(m[1]));
    }
...
const calibrate = {
  ...
  async execute(projectDir, ctx) {
    ...
    const { open } = calibrateFollowUpState(projectDir, target);
  ...
  preChecks(projectDir, ctx) {
    ...
    return [checkCalibrateTargetOption(ctx)];
  },
  postChecks(projectDir, ctx) {
    return [
  ...
  followUp(projectDir, ctx) {
    ...
    const { open, highest } = calibrateFollowUpState(projectDir, target);
    if (open) return null;
