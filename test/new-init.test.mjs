...
`);
    ...
    assert.ok(existsSync(join(dir, "tests/package.json")), "harness: package.json");
    assert.ok(existsSync(join(dir, "tests/tsconfig.json")), "harness: tsconfig.json");
    assert.ok(existsSync(join(dir, "tests/playwright.config.ts")), "harness: playwright.config.ts");
    ...
    assert.match(spec, /generated by `sdlc run ratify`/);
    assert.ok(existsSync(join(dir, "spec/domains/.gitkeep")));
...
test("init never rewrites, or stages, a hand-edited tests/acceptance/not-testable.yaml", async () => {
  // not-testable.yaml starts as pipeline content (installed once, absent-only) but
  // becomes project content the moment a team writes a real entry into it: unlike
  // .claude/settings.json or the persona briefs, init must never refresh it again, and
  // — the same reasoning as the .gitignore test above — must never sweep a team's own
  // uncommitted edit to it into an unrelated init commit either.
  const prevEgressNames = process.env.SDLC_EGRESS_NAMES;
  const egressDir = mkdtempSync(join(tmpdir(), "sdlc-egress-"));
  const emptyList = join(egressDir, "empty-egress-names.txt");
  writeFileSync(emptyList, "");
  ...
  try {
    ...
    const cfgPath = join(mkdtempSync(join(tmpdir(), "sdlc-cfg-")), "example.yaml");
    writeFileSync(cfgPath, projectConfig(pack, "main"));
    const dir = join(mkdtempSync(join(tmpdir(), "sdlc-harness-edit-")), "example-service");
    await newProject({ dir, from: cfgPath });

    const notTestablePath = join(dir, "tests", "acceptance", "not-testable.yaml");
    const original = readFileSync(notTestablePath, "utf8");
    const handEdited = original.replace("criteria: []",
      'criteria:\n  - { id: R-1.4, version: 1, reason: "no UI path to this state" }');
    writeFileSync(notTestablePath, handEdited);

    // Force `changed: true` on this init for a reason that has nothing to do with the
    // hand edit, so the commit path actually runs (see the .gitignore test above for
    // why this matters).
    rmSync(join(dir, ".sdlc", "personas", "tech-lead.md"), { force: true });
    ...
    assert.equal(r.changed, true, "the restored persona brief is a real change");
    assert.equal(readFileSync(notTestablePath, "utf8"), handEdited, "the hand edit is never rewritten");
    assert.equal(git(["diff", "--name-only"], dir), "tests/acceptance/not-testable.yaml", "the only unstaged change is the hand edit");
    assert.equal(git(["diff", "--cached", "--name-only"], dir), "", "nothing is staged");
  } finally {
    if (prevEgressNames === undefined) delete process.env.SDLC_EGRESS_NAMES;
    else process.env.SDLC_EGRESS_NAMES = prevEgressNames;
