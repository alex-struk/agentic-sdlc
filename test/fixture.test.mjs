...
test("fixture project: create, fill constitution, checks green, propose, approve, site", async () => {
  ...
  // Isolate the egress name list from this machine's real one, same as the egress
  // unit test does: point SDLC_EGRESS_NAMES at an existing-but-empty file so it wins
  // the lookup outright rather than falling through to a developer's real
  // ~/.config/agentic-sdlc/egress-names.txt. With an empty list the egress check
  // passes with a warning, which is expected here.
