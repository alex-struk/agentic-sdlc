...
A `return` verdict carries no conditions. Return the whole proposal when the recovery is wrong broadly — the domain was read against the wrong entry point, several criteria are missing evidence, the shape of what came back is not usable. When it is one row in an otherwise sound domain, `recovery-wrong` says the same thing about that row without holding the others back. Instead, say plainly what archaeology has to go back and change: which criteria are missing evidence, which statements misdescribe what the code actually does, which citations do not hold up — specific enough that the next archaeology run knows exactly what to redo, not just that something was wrong.
