...
- `recovery-wrong <ID>: <what the evidence actually shows>` — the row does not record the old application's behaviour at all. Its statement, its citations and its given/when/then describe something the application does not do, so there is no correct statement to `edit` to, and `defect` and `obsolete` would both assert the row is an accurate record of the old system when it is not. The criterion is sent back to `archaeology` to be recovered again, carrying this text verbatim; its confidence drops to `open` so it cannot be minted while it is out, and it is taken out of the closing loop until it comes back — every other criterion in the domain ratifies in the same pass. The text is the whole value of the ruling: say what was mis-recovered and what the evidence actually shows, specifically enough that the next recovery run knows what to read again. Use it when the *evidence* is wrong; use `spike` when the evidence is sound and the *decision* is not.
...
A `return` verdict carries no conditions. Use it when the recovery is wrong broadly — the domain was read against the wrong entry point, several criteria are missing evidence, the shape of what came back is not usable — because then there is no sound remainder to release; when it is one row in an otherwise sound domain, `recovery-wrong` says the same thing about that row without holding the others back. A return says nothing in the condition grammar, so say plainly in the rationale what archaeology has to go back and change: which criteria are missing evidence, which statements misdescribe what the code actually does, which citations do not hold up — specific enough that the next archaeology run knows exactly what to redo, not just that something was wrong.
...
Once the acceptance suite runs against the old application (`calibrate`), a criterion whose test fails comes back as a proposal named `calibrate-<target>-<n>`, listing the criterion, the test that failed and the failure message. Every failure on that page has already been looked at by the reviewer, who sends the ones caused by the project's own test harness back to be fixed there; what reaches this persona is a question about the product. The tests are blind — written from the criteria alone, by an agent that never saw the application — so a failure means one of exactly three things, and this persona says which. One condition line per failing criterion:
