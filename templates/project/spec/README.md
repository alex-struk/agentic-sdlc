...
- The separator between the four fields is the middle dot `·` (U+00B7). A plain hyphen
  surrounded by spaces (` - `) is also accepted.
...
spec says it should not — needs either a `replaces` (the ID of the corrected criterion) or a
`note` saying explicitly that there is no replacement yet.
